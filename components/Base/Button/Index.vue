<template>
  <button
      :class="[{ 'flex-row-reverse': iconPosition === 'left' }, buttonVariants[variant], buttonRadius[radius], $attrs.class]"
      class="flex-y-center px-4 py-1 active:scale-95 transition-transform duration-150 relative"
    ...
    <div :class="{ 'opacity-0': loading }">
      ...
      <i :class="[icon, 'text-lg align-middle']"/>
    </div>
...
import {buttonRadius, buttonVariants} from "~/data/variants";
...
const loaderPathClass = computed(() => {
  if (["primary"].includes(props.variant)) {
    return "fill-white";
  } else if (["info"].includes(props.variant)) {
    return "fill-gray-100";
  } else if (["outline"].includes(props.variant)) {
    return "fill-dark dark:fill-primary";
  }
  return "fill-white";
});
